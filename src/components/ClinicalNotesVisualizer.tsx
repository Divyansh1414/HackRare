...
export const UploadPage: React.FC = () => {
  ...
  const extractHPOTerms = async () => {
    ...
    try {
      ...
      // Mock extracted terms based on the text content
      const mockExtractedTerms = [];
      
      // Check for some common terms in the text
      if (clinicalText.toLowerCase().includes('headache')) {
        mockExtractedTerms.push({ id: 'HP:0002315', name: 'Headache' });
      }
      if (clinicalText.toLowerCase().includes('seizure')) {
        mockExtractedTerms.push({ id: 'HP:0001250', name: 'Seizure' });
      }
      if (clinicalText.toLowerCase().includes('fever')) {
        mockExtractedTerms.push({ id: 'HP:0001945', name: 'Fever' });
      }
      if (clinicalText.toLowerCase().includes('nausea')) {
        mockExtractedTerms.push({ id: 'HP:0002018', name: 'Nausea and vomiting' });
      }
      if (clinicalText.toLowerCase().includes('ataxia')) {
        mockExtractedTerms.push({ id: 'HP:0001251', name: 'Ataxia' });
      }
      if (clinicalText.toLowerCase().includes('dyspnea')) {
        mockExtractedTerms.push({ id: 'HP:0002094', name: 'Dyspnea' });
      }
      if (clinicalText.toLowerCase().includes('intellectual disability')) {
        mockExtractedTerms.push({ id: 'HP:0001249', name: 'Intellectual disability' });
      }
      if (clinicalText.toLowerCase().includes('macrocephaly')) {
        mockExtractedTerms.push({ id: 'HP:0000256', name: 'Macrocephaly' });
      }
      if (clinicalText.toLowerCase().includes('large face')) {
        mockExtractedTerms.push({ id: 'HP:0000154', name: 'Large face' });
      }
      if (clinicalText.toLowerCase().includes('nystagmus')) {
        mockExtractedTerms.push({ id: 'HP:0000639', name: 'Nystagmus' });
      }
      if (clinicalText.toLowerCase().includes('ptosis')) {
        mockExtractedTerms.push({ id: 'HP:0000508', name: 'Ptosis' });
      }
