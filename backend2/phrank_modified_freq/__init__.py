...
class Phrank:
    ...
    def rank_diseases(self, patient_phenotypes_freq):
        disease_scores = []
        for disease in self._disease_pheno_map:
            if self._disease_gene_map[disease]:
                disease_phenos = self._disease_pheno_map.get(disease, set([]))
                score = self.compute_phenotype_match(patient_phenotypes_freq, disease_phenos)
                disease_scores.append((score, disease))
        disease_scores.sort(reverse=True)
        return disease_scores
    ...
    def compute_phenotype_match(self, patient_phenotypes_freq, query_phenotypes):
        all_patient_phenotypes = closure(list(patient_phenotypes_freq['Phenotype']), self._child_to_parent)
        all_query_phenotypes = closure(query_phenotypes, self._child_to_parent)
        similarity_score = 0
        for phenotype in all_patient_phenotypes & all_query_phenotypes:
            if phenotype in patient_phenotypes_freq['Phenotype']:
                frequency = patient_phenotypes_freq.loc[patient_phenotypes_freq['Phenotype'] == phenotype, 'Frequency'].values[0]
                frequency_weight = self.get_frequency_weight(frequency)
            else:
                frequency_weight = 1
            similarity_score += self._marginal_IC.get(phenotype, 0) * frequency_weight
        return similarity_score

    def get_frequency_weight(self, frequency):
        frequency_weights = {
            "Obligate (100%)": 1.5,
            "Very frequent (99-80%)": 1.4,
            "Frequent (79-30%)": 1.3,
            "Occasional (29-5%)": 1.2,
            "Very rare (<4-1%)": 1.1,
            "Excluded (0%)": 0
        }
        return frequency_weights.get(frequency, 1.0)  # Default to 1.0 if frequency is not specified
